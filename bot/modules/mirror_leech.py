...
async def _mirror_leech(client, message, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    if args['-cine']:
        ...
        if 'cine' not in user_dict:
            await sendMessage(message, 'Cine Drive not configured! Configure it using /rclone command')
            await delete_links(message)
            return
        up_path = user_dict['cine']
        if not await aiopath.exists(f"{getcwd()}/{up_path}"):
